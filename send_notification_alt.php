...
// 設置日誌文件
$logFile = 'alt_form_submissions.log';
...
// 郵件發送設置
$to = "fgchen@gmail.com";
$from = "webmaster@" . (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : "example.com");
...
try {
    ...
    if (!$mailSuccess) {
        ...
        if ($error) {
            file_put_contents($logFile, "郵件發送失敗，錯誤信息: " . print_r($error, true) . "\n", FILE_APPEND);
        } else {
            file_put_contents($logFile, "郵件發送失敗，無詳細錯誤信息\n", FILE_APPEND);
        }
        
        // 記錄PHP mail()相關設置
        file_put_contents($logFile, "PHP mail設置:\n", FILE_APPEND);
        file_put_contents($logFile, "SMTP設置: " . ini_get('SMTP') . "\n", FILE_APPEND);
        file_put_contents($logFile, "smtp_port設置: " . ini_get('smtp_port') . "\n", FILE_APPEND);
        file_put_contents($logFile, "sendmail_path設置: " . ini_get('sendmail_path') . "\n", FILE_APPEND);
    }
} catch (Exception $e) {
